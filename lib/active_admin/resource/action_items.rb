      ...
      def add_default_action_items
        ...
        add_action_item :except => [:new, :show] do
          if controller.action_methods.include?('new') && authorized?(ActiveAdmin::Auth::CREATE, active_admin_config.resource_class)
            link_to(translation_per_model(active_admin_config.resource_class, "new_model", active_admin_config.resource_label), new_resource_path)
          end
        end
        ...
        add_action_item :only => :show do
          if controller.action_methods.include?('edit') && authorized?(ActiveAdmin::Auth::UPDATE, resource)
            link_to(translation_per_model(active_admin_config.resource_class, "edit_model", active_admin_config.resource_label), edit_resource_path(resource))
          end
        end
        ...
        add_action_item :only => :show do
          if controller.action_methods.include?("destroy") && authorized?(ActiveAdmin::Auth::DESTROY, resource)
            link_to(translation_per_model(active_admin_config.resource_class, "delete_model", active_admin_config.resource_label),
