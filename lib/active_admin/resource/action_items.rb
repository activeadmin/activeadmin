  ...
  class Resource
    module ActionItems
      ...
      def add_default_new_action_item
        displayable = -> { controller.action_methods.include?("new") && authorized?(ActiveAdmin::Auth::CREATE, active_admin_config.resource_class) }
        add_action_item :new, only: :index, if: displayable do
      ...
      def add_default_edit_action_item
        displayable = -> { controller.action_methods.include?("edit") && authorized?(ActiveAdmin::Auth::UPDATE, resource) }
        add_action_item :edit, only: :show, if: displayable do
          localizer = ActiveAdmin::Localizers.resource(active_admin_config)
      ...
      def add_default_show_action_item
        displayable = -> { controller.action_methods.include?("destroy") && authorized?(ActiveAdmin::Auth::DESTROY, resource) }
        add_action_item :destroy, only: :show, if: displayable do
          localizer = ActiveAdmin::Localizers.resource(active_admin_config)
