...
module ActiveAdmin
  ...
  class Resource
    module ActionItems
      ...
      def add_default_action_items
        ...
        add_action_item :new, only: :index do
          if controller.action_methods.include?('new') && authorized?(ActiveAdmin::Auth::CREATE, active_admin_config.resource_class)
            link_to i18n(:new), new_resource_path
          end
        end
        ...
        add_action_item :show, only: :show do
          if controller.action_methods.include?('edit') && authorized?(ActiveAdmin::Auth::UPDATE, resource)
            link_to i18n(:edit), edit_resource_path(resource)
          end
        end
        ...
        add_action_item :destroy, only: :show do
          if controller.action_methods.include?('destroy') && authorized?(ActiveAdmin::Auth::DESTROY, resource)
            link_to i18n(:destroy), resource_path(resource),
      ...
      def i18n(action)
        I18n.t("active_admin.#{active_admin_config.resource_name.singular}.#{action}_model", model: active_admin_config.resource_label, defaults: ["active_admin.#{action}_model"])
      end
    end
  end
