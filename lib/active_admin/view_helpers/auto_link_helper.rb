  ...
  module ViewHelpers
    module AutoLinkHelper
      ...
      def auto_url_for(resource)
        ...
        if config.controller.action_methods.include?('show') && authorized?(ActiveAdmin::Auth::READ, resource)
          url_for config.route_instance_path resource
        elsif config.controller.action_methods.include?('edit') && authorized?(ActiveAdmin::Auth::UPDATE, resource)
