    ...
    def has_many(assoc, options = {}, &block)
      ...
      html = with_new_form_buffer do
        template.content_tag :div, class: ["has_many #{assoc}", display_style].compact.join(" ") do
          ...
          inputs_html = template.content_tag(:ol, inputs_html, class: 'has_many_fieldsets') if @already_in_an_input_block
      ...
      html = template.content_tag(:li, html, class: 'input has_many_input') if @already_in_an_inputs_block
