...
Feature: JavaScript UI Components
  ...
  Scenario: Modal Escape does not close the underlying dropdown
    Given 2 posts exist
    And an index configuration of:
      """
      ActiveAdmin.register Post do
    ...
    When I check the 1st record
    And I press "Batch Actions"
    Then I should see the batch action :set_starred "Set Starred"
    When I click "Set Starred"
    Then I should see "Toggle Starred"
    And I should see the batch action :set_starred "Set Starred"
    When I press the "Escape" key
    Then I should not see "Toggle Starred"
    And I should see the batch action :set_starred "Set Starred"
