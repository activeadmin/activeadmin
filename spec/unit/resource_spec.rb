  ...
  RSpec.describe Resource do
    ...
    describe "permitted_params" do
      context "when not defined" do
        it "returns the expected array" do
          expect(config.permitted_params).to be_empty
        end
      end

      context "when defined" do
        let(:expected_array) { [:resource_attribute] }

        before do
          config.permitted_params = expected_array
        end

        it "returns the expected array" do
          expect(config.permitted_params).to eq(expected_array)
        end
      end
    end

    describe "build_form_with_permitted_params_only" do
      context "when not defined" do
        it "returns false as default value" do
          expect(config.build_form_with_permitted_params_only).to be false
        end
      end
      ...
      context "when defined" do
        ...
        it "returns the defined value" do
          expect(config.build_form_with_permitted_params_only).to be true
        end
      end
    end
    ...
    describe "permit_params" do
      context "when argument is an array" do
        context "when build_form_with_permitted_params_only is set to true" do
          ...
          it "sets the correct value to permitted_params" do
            expect(active_admin_config.permitted_params).to eq([:name])
          end
        end

        context "when build_form_with_permitted_params_only is not set or is false" do
          let(:active_admin_config) do
            namespace.register(Category) do
              config.build_form_with_permitted_params_only = false

              permit_params [:name]
            end
          end

          it "returns the default value of permitted_params" do
            expect(active_admin_config.permitted_params).to eq([])
          end
        end
      end

      context "when argument is a block" do
        context "when build_form_with_permitted_params_only is set to true" do
          let(:active_admin_config) do
            namespace.register(Category) do
              config.build_form_with_permitted_params_only = true
