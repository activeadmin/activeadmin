...
describe ActiveAdmin::Filters::ResourceExtension do
  ...
  it "should return the defaults if no filters are set" do
    expect(resource.filters.keys).to match_array([
      :author, :body, :category, :created_at, :custom_created_at_searcher, :custom_title_searcher ,:position, :published_date, :starred, :taggings, :title, :updated_at
    ])
  end
  ...
  it "should return the defaults without associations if default association filters are disabled on the namespace" do
    ...
    expect(resource.filters.keys).to match_array([
      :body, :created_at, :custom_created_at_searcher, :custom_title_searcher, :position, :published_date, :starred, :title, :updated_at
    ])
  end
  ...
  describe "adding a filter" do
    ...
    it "should preserve default filters" do
      ...
      expect(resource.filters.keys).to match_array([
        :author, :body, :category, :count, :created_at, :custom_created_at_searcher, :custom_title_searcher, :position, :published_date, :starred, :taggings, :title, :updated_at
      ])
    end
