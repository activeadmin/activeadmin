...
If you want to customize the `user` passed to Pundit policies and scopes as written in https://github.com/elabs/pundit#customize-pundit-user,
update the configuration in the Active Admin. The default `user` passed is the one specified in [Authentication](https://github.com/activeadmin/activeadmin/blob/master/docs/1-general-configuration.md#authentication).

```ruby
# Method name specified in your ApplicationController
config.pundit_user_method = :pundit_user
```
