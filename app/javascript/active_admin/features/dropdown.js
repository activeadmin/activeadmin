...
const menuKeydown = function(event) {
  ...
  if (event.key === "ArrowDown") {
    event.preventDefault()
    focusItem(activeDropdown.menu, "next")
  } else if (event.key === "ArrowUp") {
    event.preventDefault()
    focusItem(activeDropdown.menu, "prev")
  } else if (event.key === "Home") {
    event.preventDefault()
    focusItem(activeDropdown.menu, "first")
  } else if (event.key === "End") {
    event.preventDefault()
    focusItem(activeDropdown.menu, "last")
  }
}

const toggleKeydown = function(event) {
  if (event.key !== "Enter" && event.key !== " " && event.key !== "ArrowDown" && event.key !== "ArrowUp" && event.key !== "Escape") {
    return
  }
